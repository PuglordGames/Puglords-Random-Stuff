...
public class WarperID2Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		if (entity == null)
			return;
		if (PuglordsRandomStuffModVariables.MapVariables.get(world).warper2placed == true) {
			if (entity instanceof Player _player && !_player.level().isClientSide())
				_player.displayClientMessage(Component.literal("A warper of this ID is already placed!"), true);
			if (entity instanceof Player _player)
				_player.closeContainer();
		} else {
			world.setBlock(BlockPos.containing(x, y, z), PuglordsRandomStuffModBlocks.WARPER_FULL_2.get().defaultBlockState(), 3);
			if (entity instanceof Player _player)
				_player.closeContainer();
		}
	}
